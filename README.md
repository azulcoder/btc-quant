...
- **`btcquant/`** — the engine (pure, typed, tested; the source of truth):
  ...
  - `orderflow.py` — **optional** (`requirements-collector.txt`): event-time **order-flow bars** from the recorded tick store (day files and/or the `hf://` parquet mirror) in the SAME DataFrame contract `features.py`/`backtest.py` already consume, so `walk_forward`/`cpcv`/DSR/PBO read them with **zero harness change**. CVD + size-bucketed delta (real aggressor flags), OFI (Cont–Kukanov–Stoikov 2014, labeled a *1s snapshot approximation*), weighted mid (Stoikov 2018 first-order form, *not* the fitted micro-price), VPIN (Easley–López de Prado–O'Hara 2012, volume clock, *contested interpretation carried*), depth-imbalance slope (Næs–Skjeltorp 2006), liquidation intensity. **Features only — it never ranks, scores, or implies predictive power**, and gaps stay gaps: every bar carries `coverage`/`gap_ms`/`is_gap`/`ret_spans_gap`/`segment`, and a bar inside a collector outage is NaN, never 0.0 and never forward-filled. Liveness is witnessed **per leg** (trades gate the trade columns, depth gates the book columns), so a venue whose trade leg dies under a live book reads *unknown* rather than a tradeable flat zero; the volume clock and the OFI snapshot pairing are anchored to the UTC day, so a bar does not change value when asked for inside a wider window; and two symbols on one venue are a hard error, never a silent sum (`symbol=`). Machine-readable provenance per column (`PROVENANCE` / `provenance_table()`), with a mandatory non-empty `approximation` field. `make orderflow-smoke` runs it end-to-end into the deflation stack (bars → `features.atr`/`realized_vol` → `walk_forward` + `cpcv` → DSR/PBO/MinBTL). Measured on the 18-day bybit window where one instrument has both a trade and a book leg (2026-07-05..07-22, 1h): a 432 × 52 frame of which **52.1 % of wall time is a feed hole** and only **27 of 432** bars are fully covered with a non-gap-spanning return — 0.0031 yrs of clean bars against MinBTL(N=5) = 2.699 yrs, so the correct verdict is **INSUFFICIENT HISTORY** (1.8 % of MinBTL) and the smoke exits non-zero if the span ever *claims* to clear it. Nothing is scored, nothing is displayed, and **nothing here is research-ready** — the pipe is proven, the history is not (`RESEARCH-orderflow-runlog.md`). **Since M7 it can also read the PUBLIC BINANCE ARCHIVE** (`scripts/ingest_vision.py` -> `data/vision/`, DESIGN §3d): Binance's own published `aggTrades`, the SAME stream and SAME aggTradeId space the `binancef` collector leg already records, so overlap dedups **exactly** on `(exchange,symbol,trade_id)` and gaps are found by **ID continuity** rather than by a timestamp guess (measured on 2026-08-01: 399,219 rows, set difference **0 both ways**, **0 mismatches** on ts/price/qty/side). **Gap 1 therefore SPLITS rather than closes: `aggTrades` is TRADES ONLY.** Trade-derived families (CVD, footprint, size-bucketed delta, VPIN) reach 2,406 published days = 6.587 yrs = **244 % of MinBTL(5)**; OFI, weighted mid, depth-imbalance slope and walls gain **nothing** and stay at **1.8 %** — the archive publishes no book snapshots, and every book column is NaN on every archive bar by construction. Reading archive rows is always an explicit opt-in (`source=("local","hf","vision")`, for `volume_buckets` too); the default `source="auto"` is recorded-only, archive days never count toward the `sec_readiness` MinBTL countdown, and `make check-vision` grades the archive partition with the same duplicate-`trade_id` FAIL as the recorded store — plus a **partition-containment** FAIL, because a `date=D` partition holding another day's rows would enter that day's bars, and that day may be labelled recorded (the reader refuses such a tree outright). The `attrs["history"]` fractions each name their basis: the unqualified `fraction_of_minbtl` is the SHORTEST family over days that actually contributed rows, never the requested window (`RESEARCH-vision-runlog.md` §9).
...
- **`scripts/`** — CLIs:
  ...
  - `orderflow_smoke.py` — M1 end-to-end: recorded ticks → order-flow bars → `features`/`walk_forward`/DSR/PBO/MinBTL with zero harness change. Exits non-zero if the span ever claims to clear MinBTL on this archive.
  ...
  - `ingest_vision.py` — **M7**: pulls Binance's own published `aggTrades` archive (`data.binance.vision`, no key, no third-party code) into `data/vision/` as ZSTD parquet — a **separate tree** from `data/ticks/`, never written into a recorded day file. `make vision-list` enumerates what the archive publishes and downloads nothing; `make vision-sync ARGS="--start .. --end .."` ingests. Scope is an **allowlist in code**, not a comment (`futures/um/aggTrades/BTCUSDT` only, plus a target allowlist so a vendor object can only land under the venue/symbol whose recorded leg shares its id space). Seven gates per day, and a day is not trusted until all seven pass: sha256 against the companion `.CHECKSUM`, single-entry zip, header **sniffed per file** (its presence varies per file, not per year — a year cutoff loses or invents exactly one row on scattered days), every `ts_ms` inside the day the path claims, a millisecond-unit check, `rows == distinct ids` with `max−min+1−count` recorded as `id_holes`, and a re-read of the written parquet. A 404 is `status="absent"` in `data/vision/_ledger.jsonl` with **no file at all** — never a zero-row parquet. Measured on the published listing: **2,406 days, 2019-12-31 .. 2026-08-01, zero missing**, ~41 GiB of zip → ~13 GB of parquet. **It buys trades and only trades** — see `orderflow.py` above for what that does and does not close.
  - `check_ticks.py` — L3 tick-store QA (`make check-ticks`), and since M7 the same gate definition over the archive partition (`make check-vision`): same duplicate-`trade_id` FAIL, plus a partition-containment FAIL and an ID-continuity census, and it **refuses to print a MinBTL readiness number** because archive days do not count toward it.
- **`dashboard/`** — static web terminal (no build step): the OOS leaderboard, live candles + indicators, equity/drawdown, return distribution, rolling vol/Sharpe, perp funding/basis, option-IV surface. A faithful mirror of the Python engine. Open `dashboard/index.html` or serve the folder.
  - **`terminal.html`** — the **orderflow terminal** (live-descriptive ONLY, never a backtest input) — the most honest, validation-coupled crypto order-flow **research workbench**, an observation surface that *informs* execution in a separate native engine, never an execution venue itself: footprint chart + session volume profile (POC/VAH/VAL), DOM ladder, time-and-sales tape, multi-exchange aggregated orderbook, CVD by trade-size bucket **and per exchange**, liquidation feed, mark/funding/OI header; plus the O-2 heatmap layer — session **orderbook heatmap** (time × price × resting liquidity), **spoof-pull / iceberg-refill detections** (labeled *heuristic* — patterns consistent with, not proof), and a **liquidation heatmap** (labeled *ESTIMATED (model)* — volume-weighted entry proxy × standard leverage tiers). Plus the O-3 structure layer — historical chart with **no-peek bar replay**, classical 30-min **TPO / Market Profile**, composite volume profile (labeled *bar-range approximation*), cross-venue **funding/OI table** (descriptive; carry stays off-board), **macro proxies** (HIP-3 index/commodity perps + PAXG — labeled proxies, *no CME feeds*), and **BYOD tick replay**: `terminal.html?replay=byod` re-plays your own collector recordings through the same panels. The O-4 intelligence layer adds a 720-symbol **VWAP-deviation screener** + **RSI heatmap** (bybit, labeled *24h VWAP = turnover/volume*), a **Deribit options widget** (IV smile/term/heatmap, PCR, max pain, **unsigned Σ|Γ|·OI** — dealer sign unknowable), **Hyperliquid whale watchlist** (public on-chain state — *facts, not signals*), a 9-rule **alert engine** and a 9-read **mechanical confluence board** — both labeled *un-validated descriptive, NOT signals* (forward IC of board signals ≈ 0, RESEARCH-ic-runlog). The O-5 portfolio layer completes the map: **trade journal** (your own logged trades — Tharp R-multiple stats, *NOT a backtest*; CSV export/import), **calendar returns**, **Polymarket BTC panel** (*crowd-implied probabilities*), **Tree-of-Alpha news feed**, and a **local-mirror econ calendar** (`make econ` — the origin has no CORS). The I-1 **Institutional Auction Suite** goes beyond the map: **tick-exact volume profiles** from the recorded store (total / buy×sell / **delta** / size-bucket; server-side DuckDB aggregation — a 2.87M-trade day in ~20ms), a **daily levels registry** with serve-time **naked-POC** derivation, session **VWAP ±1σ/±2σ bands** (AMT read: value ≈ vwap ± 1σ), **OFI (Cont–Kukanov–Stoikov) + microprice (Stoikov)** panes, footprint stacked-imbalance zones + absorption flags (*heuristic*), and **in-browser archived-day analytics**: the public terminal can pull any recorded day straight from the HF dataset (vendored hyparquet+fzstd, size-gated). Elite pass: sticky section nav with persisted collapse, hidden-tab/offscreen paint gating (ingestion never pauses), and the terminal fixture smoke (now 83 groups) promoted to a **CI build gate**. The T-4 Wave 1 **Truth** pass makes the instrument honest about itself: a venue's non-JSON keepalive reply is now a **control frame, not a dropped frame** (the health chip had been permanently amber on a healthy page), socket **close codes are captured** instead of discarded — which measured the Bybit leg drops to be an abnormal **transport** close (1006, no reason, tab visible), *not* the ping timeout the code had assumed — the tape carries a **$10k floor with the sub-floor volume summarised, never discarded**, the news feed gains a **visible crypto/all relevance toggle** over a measured evidence ladder, and three duplicated "collector API offline" paragraphs collapse into **one local-only strip** that refuses to fold a panel which still has data. The T-3 **Tape & Ladder** pass turns the tape into an aggr-style aggregated reader — the enabled venue legs merged into one **size-tiered** tape (configurable $-notional tiers, a **big-print rail**, an optional audio cue *(a UX aid, not a signal)*), with same-price bursts merged into a `×N` VWAP block (never across venues — that would fake a single print) — and rebuilds the DOM ladder on the T-2 full local books: log-scaled **depth bars**, cumulative depth, **wall markers**, a **trade imprint** (executed buy/sell volume painted at each price level), spread/mid/imbalance readouts, and an optional same-quote **aggregated ladder** (display-only; cross-venue depth is loudly caveated and non-USDT legs are excluded, not rescaled). The T-2 **Venue Matrix** pass runs seven independently enable/disable-able venue×market legs — bybit linear+spot, binance futures+spot, okx swap+spot, coinbase spot — each maintaining a **full local order book** (Binance snapshot+diff with spot `U/u` vs futures `pu`-chain continuity; OKX `books` with seqId/prevSeqId chaining after the keyless wire was measured to send `checksum: 0` on every frame; Coinbase full snapshot + `l2update`), with **honest counted resyncs** (a desync clears the book, never patches it), a leg-aware aggregated order book, and a **spot-vs-perp CVD** strip. The T-1 **Trader's Edge** pass makes the symbol **runtime-switchable** (Bybit linear universe; every WS leg re-derives its venue id, probes listability, and degrades to an honest *no-leg* chip — store-backed panels state their BTCUSDT-only reality), and adds per-bar **Δmin/Δmax/Δ%** footprint rows + **unfinished-auction** markers, a **tape-intensity** gauge (z vs session baseline — *burst ≠ signal*), a resting-order **walls ledger** (*book-history bookkeeping, not intent*), **VPIN** (Easley–López de Prado–O'Hara 2012, computed from real aggressor flags; *toxicity interpretation contested — a series, not a claim*), a Dalton **opening-type** chip (*descriptive session read, not a signal*), a **key-levels strip** (registry levels + live Initial Balance, footprint overlay), a **basis/funding** intraday mini-chart, and a **⌘K command palette** with workspace presets. Keyless WS (Bybit primary + Binance depth + OKX + Coinbase tape). Design + honesty rails: `DESIGN-orderflow-terminal.md`. Strategic assessment + staged roadmap (where it stands, the moat, the honest gaps, what to build next and what to refuse): `STRATEGY.md`.
